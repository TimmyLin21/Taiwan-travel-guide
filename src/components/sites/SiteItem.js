  ...
  return (
    <SiteCard>
      <button type="button" onClick={goDetailHandler}>
        { pictureUrl ? <img src={pictureUrl} alt={pictureDescription} /> : <img src={comingSoon} alt="Not found!" />}
      </button>
      <div>
        <h4>{title}</h4>
        <SiteInfo location={location} openHour={openHour} time={time} phone={phone}/>
      </div>
    </SiteCard>
  )
};
